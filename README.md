...
> 🐤 一个基于多智能体强化学习（Multi-Agent Actor-Critic, MAAC）的老鹰与小鸡追踪仿真项目

环境中包含 **🦅 老鹰（UAV）**、**🐤 小鸡（Chick）** 与 **🐔 母鸡（Hen）**，通过奖励与惩罚设计推动老鹰学习高效追踪小鸡并避免进入母鸡的"禁区"。
...
- 🤝 **多老鹰协作与通信** - 基于离散动作控制移动与转向
- 🐤 **小鸡捕获判定** - `capture_radius` 判定与小鸡覆盖率统计
- 🐔 **母鸡安全机制** - 安全半径惩罚与边界惩罚
...
│   ├── 📂 agent/                   # 智能体模块
│   │   ├── 🦅 uav.py               # 老鹰（UAV）行为、观测与奖励分量计算
│   │   ├── 🐔 protector.py         # 母鸡移动与边界约束
│   │   └── 🐤 target.py            # 小鸡移动与边界约束
│   ├── 📂 models/                  # 神经网络模型
...
**演示模式（Run Mode）** 是一个无需训练权重的快速预览模式，使用内置的启发式算法控制老鹰行为。
...
- 📍 `u_xy/u_xy0.csv` - 老鹰轨迹数据
- 📍 `t_xy/t_xy0.csv` - 小鸡轨迹数据
- 📍 `p_xy/p_xy0.csv` - 母鸡轨迹数据
- 📊 `covered_target_num/covered_target_num0.csv` - 小鸡覆盖率统计
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `environment.n_uav` | 🦅 老鹰数量 | 3 |
| `environment.m_targets` | 🐤 小鸡数量 | 10 |
| `environment.n_protectors` | 🐔 母鸡数量 | 3 |
| `uav.dp` | 👁️ 老鹰探测半径 | 200 |
| `target.capture_radius` | 📍 小鸡捕获半径 | 120 |
| `protector.safe_radius` | ⚠️ 母鸡安全半径 | 200 |
...
- `dp`：👁️ 观测/追踪半径（影响小鸡跟踪奖励触发）
- `alpha` / `beta` / `gamma` / `omega`：⚖️ 奖励分量权重（线性组合为总奖励）

### 🐔 母鸡配置 (`protector`)
- `safe_radius`：⚠️ 母鸡的安全半径（禁区半径），老鹰进入会受罚
  - 值越大越容易触发惩罚；与动画中母鸡"阻挡手臂"的半长度一致

### 🐤 小鸡配置 (`target`)
- `capture_radius`：📍 小鸡捕获半径（老鹰到小鸡距离 ≤ 此值即捕获）
  - 判定逻辑在 `environment.step`，被捕获小鸡会被隐藏
...
| 分量 | 符号 | 权重 | 触发条件 | 作用 |
|------|------|------|----------|------|
| 🐤 小鸡跟踪奖励 | `target_tracking_reward` | α=0.6 | 距离小鸡 ≤ dp | 鼓励靠近小鸡 |
| 🚫 重复追踪惩罚 | `duplicate_tracking_punishment` | β=0.2 | 老鹰间距 < dc | 避免多老鹰扎堆 |
| ⚠️ 边界惩罚 | `boundary_punishment` | γ=0.2 | 靠近地图边界 | 保持安全距离 |
| 🐔 母鸡惩罚 | `protector_punishment` | ω=0.5 | 距离 < safe_radius | 避开母鸡防御区域 |
...
本项目实现了 **老鹰碰到母鸡手臂后被物理弹开** 的效果。

### 🎯 工作原理

当老鹰靠近母鸡的防御手臂时：
1. **检测碰撞**: 计算老鹰到手臂线段的最近距离
2. **触发弹开**: 当距离 < `arm_thickness` 时触发
3. **物理推离**: 沿法线方向推离老鹰，推离距离 ≤ `knockback`
