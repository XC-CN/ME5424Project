...
class ActorCritic:
    ...
    def update(self, transition_dict):
        ...
        rewards_normalized = torch.clamp(rewards_normalized, min=-3.0, max=3.0)
        
        # 计算td_target和td_delta（使用归一化后的奖励）
        td_target_normalized = rewards_normalized + self.gamma * self.critic(next_states).detach()
        td_target_normalized = torch.clamp(td_target_normalized, min=-2.0, max=2.0)
        ...
        td_delta_clipped = torch.clamp(td_delta_normalized.detach(), min=-3.0, max=3.0)
        ...
        torch.nn.utils.clip_grad_norm_(self.actor.parameters(), max_norm=0.35)
        torch.nn.utils.clip_grad_norm_(self.critic.parameters(), max_norm=0.35)
        torch.nn.utils.clip_grad_value_(self.critic.parameters(), clip_value=0.7)
